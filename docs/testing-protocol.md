...
- `cache-cliff-warn.sh` writes `HANDOFF-stats.json` to the agent CWD and touches a `banner-fired` sentinel — **test mode only**.
...
| ~200s | Model finishes writing HANDOFF.md, Stop fires |
| ~200s | warn.sh fires: checks sentinel, emits systemMessage banner, writes HANDOFF-stats.json |

## Loop Harness

Use `/loop 15s` with the standard cache-cliff test harness prompt to automate detection. The loop polls every minute (cron minimum) and stops itself when `HANDOFF-stats.json` appears, removing the test flag automatically.
...
Each tick:
1. If HANDOFF-stats.json exists in CWD: read it, remove test flag, report stats, stop loop.
2. Else if HANDOFF.md modified in last 10 minutes: "HANDOFF.md written at HH:MM — waiting for banner". Continue.
...
| `handoff_cost_in` | Input tokens consumed generating HANDOFF.md |
| `handoff_cost_out` | Output tokens consumed generating HANDOFF.md |
...
**Tuning lever**: `handoff_cost_out` is the primary efficiency metric. A verbose HANDOFF.md costs more output tokens. The rewake prompt's section template and any word-budget hint directly control this.
...
| Banner fires but no HANDOFF-stats.json | Test flag was removed before warn.sh ran | Keep test flag in place until loop detects stats |
